...
const GOTO: Record<string, Partial<AppState>> = {
  ...
  // The panel rows address a panel by GRAMMAR COMPONENT, not by `el_id`: every artifact names
  // its own elements, so `[data-el="..."]` here would be a content string in app source and
  // would stop matching the next time the pipeline publishes. `data-panel` is the renderer's
  // own name for what it is, and it is the same on every narrative.
  'autopsy.panel.claim-map': autopsy('mbg-stop', { focus: '[data-panel="claim_map"]' }),
  'autopsy.panel.scale': autopsy('mbg-stop', { focus: '[data-panel="scale_check"]' }),
  'autopsy.panel.money.off': autopsy('mbg-stop', { focus: '[data-panel="money_flow"]' }),
  'autopsy.panel.money.on': autopsy('mbg-stop', { moneyStopped: true, focus: '[data-panel="money_flow"]' }),
  'autopsy.panel.incidence': autopsy('mbg-stop', { focus: '[data-panel="incidence"]' }),
  'autopsy.panel.dueling': autopsy('mbg-poisoning', { focus: '[data-panel="dueling"]' }),
  ...
  'autopsy.panel.options': autopsy('mbg-stop', { focus: '[data-panel="options"]' }),
  'autopsy.panel.family': autopsy('mbg-stop', { focus: '[data-panel="family"]' }),
  'autopsy.evidence-sheet': autopsy('mbg-stop', { sheet: 'evidence' }),
  'autopsy.narration-highlight': autopsy('mbg-stop', { sent: 'v-1', focus: '[data-panel="claim_map"]' }),
