 ...
 *
 *   tsx scripts/build-replay.ts [--run <run dir> | --runs <dir,dir>] [--out <content root>]
 *
 * The archive spans more than one recorded run, so the replay does too: each narrative names the
 * run it was recorded in, and one disclosure covers them all. A run manifest lists every
 * narrative the fleet knew about, so only the ids with a recorded `slots/<id>/steps.json` in
 * that directory belong to that run; without that filter an earlier run's narratives would be
 * re-emitted under the later run's id, which is the exact misattribution this file exists to
 * prevent.
 *
 * The replay is a LABELLED REPLAY of the editorial runs (blueprint 3.4 honesty rules), so
...
function usage(message: string): never {
  ...
  console.error('usage: tsx scripts/build-replay.ts [--run <run dir> | --runs <dir,dir>] [--out <content root>]');
  process.exit(2);
}

/** Every run the published archive was recorded in. Adding a run here is the only edit needed. */
const DEFAULT_RUNS = ['pipeline/runs/run-2026-07-29', 'pipeline/runs/run-2026-08-10'];

function parseArgs(argv: string[]): { runs: string[]; out: string } {
  const flags = new Map<string, string>();
  for (let i = 0; i < argv.length; i += 2) {
    const flag = argv[i] ?? '';
    const value = argv[i + 1];
    if (flag !== '--run' && flag !== '--runs' && flag !== '--out') usage(`unknown argument "${flag}"`);
    if (value === undefined || value.startsWith('--')) usage(`${flag} needs a value`);
    if (flags.has(flag)) usage(`${flag} was given twice`);
    flags.set(flag, value);
  }
  if (flags.has('--run') && flags.has('--runs')) usage('--run and --runs are exclusive');
  const single = flags.get('--run');
  const many = flags.get('--runs')?.split(',').map((dir) => dir.trim()).filter((dir) => dir !== '');
  if (many !== undefined && many.length === 0) usage('--runs needs at least one run dir');
  const runs = many ?? (single === undefined ? DEFAULT_RUNS : [single]);
  return {
    runs: runs.map((dir) => resolve(dir)),
...
/**
 * One recorded run, read entirely from its own directory. `modelsSeen` is the shared union the
 * caller reports at top level; `constellation` is A13's filing, which lives in the content root
 * and is therefore read once by the caller rather than per run.
 */
function buildRun(
  runDir: string,
  out: string,
  constellation: { nodes: { narrative_id: string }[] },
  ...
  const stageHas: Record<'A1' | 'A2' | 'A3' | 'A4', (id: string) => boolean> = {
    A1: memberCheck(readJson<{ records: { id: string }[] }>(join(runDir, 'stages', 'A1.json')).records),
    A2: memberCheck(readJson<{ records: { id: string }[] }>(join(runDir, 'stages', 'A2.json')).records),
    A3: memberCheck(readJson<{ assignments: { id: string }[] }>(join(runDir, 'stages', 'A3.json')).assignments),
    A4: (() => {
      const ranked = readJson<{ ranked: { cluster: string }[] }>(join(runDir, 'stages', 'A4.json')).ranked;
      return (id: string) => ranked.some((r) => r.cluster.startsWith(`c-${id}-`));
    })(),
  };

  // The manifest names every narrative the fleet knew about; `steps.json` is the record of the
  // ones this run actually worked. Without the filter a narrative recorded in July replays a
  // second time under August's id, attributing work to a run that never did it.
  const narratives = runManifest.narratives
    .filter((id) => existsSync(join(runDir, 'slots', id, 'steps.json')))
    .map((id): Dict => {
      const slotDir = join(runDir, 'slots', id);
      ...
      // The judges' notes describe their verdicts, so they ride the verdict events below, not
      // the started-work slot lines: one note per role, said where it happened.
      events.push(
        ...steps
      ...
      const artifact = join(out, 'narratives', `${id}.json`);
      let token = '';
      ...
      return {
        ...
        blocked_rounds: blocks.filter((x) => x.narrative_id === id).length,
        events,
      };
    });

  return { run_id: runManifest.run_id, generated_at: runManifest.generated_at, narratives, blocks };
}
...
function main(): void {
  ...
  const built = args.runs.map((dir) => buildRun(dir, args.out, constellation, modelsSeen));
  const runIds = built.map((b) => b.run_id).sort((a, b) => a.localeCompare(b));
  const narratives = built.flatMap((b) => b.narratives);
  const blocks = built.flatMap((b) => b.blocks);
  // The archive's freshness is the newest run in it, not whichever run was listed last.
  const generatedAt = built.map((b) => b.generated_at).sort((a, b) => a.localeCompare(b)).at(-1) ?? '';

  const replay = {
    run_ids: runIds,
    generated_at: generatedAt,
    // The honest frame the UI must keep visible, both locales, no em dash, no emoji. One
    // disclosure covers every run in the record, and each narrative names its own run.
    disclosure: {
      en: `Replay of editorial run${runIds.length > 1 ? 's' : ''} ${runIds.join(' and ')}. Every step, model, verdict and block below is the recorded run it names; the indented output lines are an editorial distillation of each agent's recorded output files; on-screen pacing is compressed and not to scale.`,
      id: `Pemutaran ulang proses editorial ${runIds.join(' dan ')}. Setiap langkah, model, putusan dan blokir di bawah ini adalah rekaman asli dari proses yang namanya tertera; baris keluaran yang menjorok adalah saripati editorial dari berkas keluaran tiap agen yang terekam; tempo di layar dipadatkan dan tidak berskala.`,
    },
  ...
  const events = narratives.reduce((n, x) => n + (x.events as Dict[]).length, 0);
  console.log(
    `build-replay: ${outPath} carries ${String(runIds.length)} run(s), ${String(narratives.length)} narrative(s), ` +
      `${String(events)} event(s), ${String(replay.blocks_total)} real block record(s), ` +
      `${String(replay.models.length)} model identit(ies)`,
  );
}
